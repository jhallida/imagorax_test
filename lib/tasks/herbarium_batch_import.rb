      ...
      def Tasks.herbarium_batch_import(data_file, owner_username, deleteafteringest)
        ...
        begin
          ...
          CSV.foreach(data_file, headers:true) do |row|
            ...
            gfs = HerbariumImage.search_with_conditions catalog_number_sim: cat_num
            multivalue_row = []
            row.to_hash.map do |k,v|
              ...
              if HerbariumImage.properties[k].try :multiple?
            ...
            if gfs.size == 0
              ...
              gf = HerbariumImage.create!(multivalue_row.to_h) do |obj|
                #add a few more boilerplate metadata items
                obj.title = [cat_num]
                obj.depositor = owner.email
                obj.edit_users = [owner.email]
                obj.license = ['http://creativecommons.org/licenses/by-nc/3.0/us/']
              ...
              gf = HerbariumImage.find(gfs.first['id'])
              gf.update(multivalue_row.to_h)

              gf.title = [cat_num]
              gf.depositor = owner.email
              gf.edit_users = [owner.email]
              gf.license = ['http://creativecommons.org/licenses/by-nc/3.0/us/']
              ...
              FileSet.find((HerbariumImage.search_with_conditions id: gf.id).first['hasRelatedImage_ssim'].first).destroy
